...
public class ViewLoanActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("View Loan Item");

        spinnerLoanTable = findViewById(R.id.viewSpinnerLoanView);
        ArrayAdapter sByFilter = new ArrayAdapter(ViewLoanActivity.this,R.layout.spinner_custom,searchBY);
        spinnerLoanTable.setAdapter(sByFilter);
        loanViewSearchET = findViewById(R.id.loanViewSearchET);

        spinnerDay = findViewById(R.id.spinnerDay);
        ArrayAdapter sDays = new ArrayAdapter(ViewLoanActivity.this,R.layout.spinner_custom,days);
        spinnerDay.setAdapter(sDays);

        searchViewLoanDoc = findViewById(R.id.searchViewLoanDoc);
        ...
        AsyncGetLoanDtlSqlite asyncGetLoanDtlSqlite = new AsyncGetLoanDtlSqlite(ViewLoanActivity.this);
        asyncGetLoanDtlSqlite.execute();
    ...
    public void searchLoanList(){
        searchViewLoanDoc.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                AsyncGetLoanDtlSqliteByDay asyncGetLoanDtlSqliteByDay = new AsyncGetLoanDtlSqliteByDay(ViewLoanActivity.this);
                asyncGetLoanDtlSqliteByDay.execute();

                Message.message(ViewLoanActivity.this,"Set table within " + searchDay + " days");
            }
        });
    }

    public void filterTable(){
        spinnerLoanTable.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if(position != 0){
                    SearchPos = position;
                    ...
                    Message.message(ViewLoanActivity.this,"Search table by " + searchby);
                }
            }
    ...
    public void searchTable(){
        loanViewSearchET.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                for(int i = 0;i<listOfItemRow.size();i++){
                    ...
                    AdapterFive adapterFive = new AdapterFive(ViewLoanActivity.this, searchList);
    ...
    public class AsyncGetLoanDtlSqliteByDay extends AsyncTask<Object, String, ArrayList> {
        Context context;
        ProgressDialog progressDialog;

        public AsyncGetLoanDtlSqliteByDay(Context context) {
            this.context = context;
        }

        @Override
        protected void onPreExecute() {
            progressDialog = new ProgressDialog(ViewLoanActivity.this);
            progressDialog.show();
            super.onPreExecute();
        }
        ...
        protected ArrayList doInBackground(Object... objects) {
            ArrayList isSuccess = getLoanDTLSqliteByDay(CommonDBItem.SQLiteLoanItemDtl, searchDay);
        ...
        protected void onPostExecute(ArrayList arrayList) {
            if (progressDialog.isShowing()) {
                ...
                if (arrayList.size() > 0) {
                    ...
                    customAlertDialog.DialogWarningAlert(ViewLoanActivity.this, "Loan Item Detail",
                            "Loan item not Available").show();
                }
            }
        }
    ...
    public ArrayList getLoanDTLSqliteByDay (String strTableName,String day) {
        ArrayList arrayList = new ArrayList();
        SqliteDBHelper sqliteDBHelper = new SqliteDBHelper(ViewLoanActivity.this, getString(R.string.appdb_name), getFilesDir().getPath());

        if (sqliteDBHelper.checkTableExistence(strTableName)) {
            arrayList = sqliteDBHelper.getTableDataFromSqlite("select * from " + strTableName + " where Status = '0' and strftime('%Y', OutDate) = '2020'");
    ...
    public class AsyncGetLoanDtlSqlite extends AsyncTask<Object, String, ArrayList> {
        Context context;
        ProgressDialog progressDialog;

        public AsyncGetLoanDtlSqlite(Context context) {
            this.context = context;
        }
        ...
        @Override
        protected ArrayList doInBackground(Object... objects) {
            ArrayList isSuccess = getLoanDTLSqlite(CommonDBItem.SQLiteLoanItemDtl);
            System.out.println(isSuccess);
            return isSuccess;
        }
        ...
        protected void onPostExecute(ArrayList arrayList) {
            if (progressDialog.isShowing()) {
                ...
                if (arrayList.size() > 0) {
                    ...
                    customAlertDialog.DialogWarningAlert(ViewLoanActivity.this, "Audit ID Detail",
    ...
    public void doRecycler() {
        ...
        if (listOfItemRow.size() > 0) {
            ...
            AdapterFive adapterFive = new AdapterFive(ViewLoanActivity.this, listModel);
            recyclerView.setAdapter(adapterFive);
            adapterFive.notifyDataSetChanged();
    ...
    public ArrayList<SingleModelItem> populateViewData(int dataPosition, HashMap hashData){
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        DynaTextModel textModelItem1 =  new DynaTextModel(ViewLoanActivity.this.getResources().getColor(R.color.satoColor, getTheme()), 14);
        textModelItem1.setDrawableModel(new DynaDrawableModel(ViewLoanActivity.this.getResources().getColor(R.color.satoColor, getTheme()) , Color.WHITE));
        textModelItem1.setLayoutParam(new DynaLayoutParam(130, LinearLayout.LayoutParams.MATCH_PARENT));

        DynaTextModel textModelItem =  new DynaTextModel(ViewLoanActivity.this.getResources().getColor(R.color.satoColor, getTheme()), 14);
        textModelItem.setDrawableModel(new DynaDrawableModel(ViewLoanActivity.this.getResources().getColor(R.color.satoColor, getTheme()) ,Color.WHITE));
    ...
    public ArrayList getLoanDTLSqlite(String strTableName) {
        ArrayList arrayList = new ArrayList();
        SqliteDBHelper sqliteDBHelper = new SqliteDBHelper(ViewLoanActivity.this, getString(R.string.appdb_name), getFilesDir().getPath());

        if (sqliteDBHelper.checkTableExistence(strTableName)) {
            arrayList = sqliteDBHelper.getTableDataFromSqlite("select * from " + strTableName + " where Status = '0'");
        } else { System.out.println("Table Don't Exist"); }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                startActivity(new Intent(ViewLoanActivity.this, MenuActivity.class));
                finish();
                return true;
            default:
